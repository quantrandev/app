...
namespace VNScience.Areas.Admin.Controllers
{
    public class PostController : BaseController
    {
        ...
        // GET: Admin/Post
        [Authorize(Roles = RoleName.PostMod + "," + RoleName.SystemMod)]
        ...
        [ValidateInput(false)]
        [Authorize(Roles = RoleName.SystemMod)]
        public ActionResult CreateActivityNews(PostViewModel model)
        {
            bool isSuccess = true;
            if (!ModelState.IsValid)
                return View();
        ...
        [Authorize(Roles = RoleName.SystemMod)]
        [ValidateInput(false)]
        public ActionResult EditActivityNews(PostViewModel model)
        {
            bool isSuccess = true;
            if (!ModelState.IsValid)
                return View();
            ...
            if (!isSuccess)
                ...
                Notification.Success("Cập nhật bài viết thành công", Session);

            return RedirectToAction("Index");
        }

        [HttpGet]
        [Authorize(Roles = RoleName.Admin + "," + RoleName.SystemMod)]
        public ActionResult DetailsActivityNews(int id)
        {
            var model = postDAO.GetWithUserAndCategoryAndTags(id);
            return View(model);
        }
